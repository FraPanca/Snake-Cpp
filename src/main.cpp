...
int main() {
	...
	texture.loadFromFile("assets/images/snake.png");
	
	sf::Sprite s;
	
	s.setTexture(texture);
	
	texture_f.loadFromFile("assets/images/frutta.png");
	...
	background.loadFromFile("assets/images/campo.png");
	  ...
	  if(start==0) {
		...
		window.display();
	  	...
	  	if(k==0) { //preparazione schermata finale
			...
			fp.open("data/punteggio.txt", ios::app);
			fp << punteggio; //scrittura punteggio nel file
			fp << "\n";
			fp.close();
			
			fp.open("data/punteggio.txt", ios::in);
